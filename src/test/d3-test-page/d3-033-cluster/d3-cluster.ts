...
window.addEventListener('load', async() => {
  ...
  const cluster = d3
    .cluster()
    .size([width - 100, height - 100]) // cluster 를 계산할 영역을 지정.
  ;
  const rootNode = d3.hierarchy<ID3Custer.Data>(data); // 계층 구조의 데이터를 HierarchyNode 객체로 반환해주는 d3.hierarchy 을 사용하여 rootNode 에 저장.
