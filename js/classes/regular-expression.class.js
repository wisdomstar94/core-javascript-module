/**
 * @typedef {'allow-all' | 'allow-english' | 'allow-korean' | 'allow-enter' | 'allow-number' | 'allow-space' | 'allow-dash'} InputRule
 */

/**
 * @description 정규식 관련 함수를 제공하는 클래스 입니다.
 */
class RegularExpression {
   ...
   * @returns
   */
  getRemoveSpecialCharString(text, isSpaceRemove) {
   ...
   * @param {null | string[]} inputCharRules
   * @returns {string}
   */
  getRemoveNotInputRulesString(text, inputRules, inputCharRules) {
   ...
   * @returns {boolean}
   */
  getRemoveEnglishString(text) {
    return text.replace(new RegExp('[a-z][A-Z]', 'gi'), '');
  }

  /**
   * @function isEmail
   * @description email 형식인지 체크
   * @returns {boolean}
   */
  isEmail(text) {
   ...
   * @returns {boolean}
   */
  isExistOnlyEnglishOrNumber(text) {
    if (typeof text !== 'string') {
      return false;
    }
   ...
   * @returns {boolean}
   */
  isExistOnlyEnglishAndNumber(text) {
    if (typeof text !== 'string') {
      return false;
    }
